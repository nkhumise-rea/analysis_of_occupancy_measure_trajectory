...
"""
Stochastic (slipping actions) 
2D Gridworld of size NxN where start-state = (0,0) and goal-state = (N-1,N-1), 
...
class grid():
    ...
    def done(self, next_state): # get_done_status
        ...
        if self.step_id == 40: #15,40,60,90
            done = True
            return done*1.0
    ...
    def action_val(self,action_index):
        ...
        ##_stochasticity
        hp = 0.8 #high_probability
        lp = 0.1 #low_probability
        if self.actions[action_index] == 'up': 
            action_index = np.random.choice([0,1,2,3],1,p=[hp,lp,0.0,lp])[0]
        elif self.actions[action_index] == 'right': 
            action_index = np.random.choice([0,1,2,3],1,p=[lp,hp,lp,0.0])[0]
        elif self.actions[action_index] == 'down': 
            action_index = np.random.choice([0,1,2,3],1,p=[0.0,lp,hp,lp])[0]
        elif self.actions[action_index] == 'left': 
            action_index = np.random.choice([0,1,2,3],1,p=[lp,0.0,lp,hp])[0]
