...
"""
Deterministic 2D Gridworld of size NxN where start-state = (0,0), 
goal-state = (N-1,N-1), sink-states = [(0,3);(3,0)] and block-state = (1,1)
...
class grid():
    def __init__(self, 
        ...
        if self.reward_setting == 1: #dense
            ...
            # ##sinks
            for j in tuple([ [3,0] ]):
                self.rewards[tuple(j)] = -1
            for j in tuple([ [0,3] ]):
                self.rewards[tuple(j)] = -1

            # single_optimal_path
            optimal_path = { 0:[(0,1),(0,2),(1,2),(2,2),(3,2),(4,2),(4,3)],
    ...
    def is_terminal_state(self,current_state): #terminal_state check
        if self.rewards[tuple(current_state)] == 1: 
            return True
        elif self.rewards[tuple(current_state)] == -1: 
            return True
        else:
            return False   
    ...
    def state_tracking(self,current_state, action_index):
        ...
        # print('current_state: ',current_state==(0,0))
        ...
        # """
        block_1 = (1,1)
        if tuple(next_state) == block_1:
            if tuple(current_state) == (1,0):
                next_state = np.asarray(current_state)
            elif tuple(current_state) == (0,1):
                next_state = np.asarray(current_state)
            elif tuple(current_state) == (1,2):
                next_state = np.asarray(current_state)
            elif tuple(current_state) == (2,1):
                next_state = np.asarray(current_state)
        else:
            if next_state[0] < 0: 
