...
class gridworld():
    ...
    def main(self,
        ...
        self.tau = tau #temperature [low: exploit, high:explore]
    ...
    def get_action_stochastically(self, state):
        #sample action_index
        action_values = self.q_values[state[0],state[1]]
        exp_values = np.exp(action_values/self.tau)
        self.probs = exp_values / np.sum(exp_values,axis=0)
        action_index = random.choice(self.num_actions,p=self.probs)
        return action_index

    #greedy_action
    def get_action_deterministically(self, state):
        #select action_index w/ highest probability
        # action_index = np.argmax(self.q_values[state[0],state[1]]) #self.probs

        #random tie_breaker
        action_index = self.randargmax(self.q_values[state[0],state[1]])
        return action_index
