...
from envs.Q_learn import gridworld

class otdd():
    def __init__(self,
                 test_freq=50,states_size=[3,3],setting=1,n_eps=1000,epsilon=1.0,stop_cri=50,max_esteps=60):
    ...
    def curve_length(self,num_curve):
        trajectory_dataset,steps_col,score_col,con_eps,con_step,num_eval,state_dis,regret_col,state_value_col = self.agent.main(
                                    ...
                                    epsilon=self.epsilon, #[1: random | 0: greedy ]
                                    )
        ...
        if num_eval == -1: # failed_to_converge
            ...
            outcome = 'Fail'
            self.monot(ds_stt_col,steps_col,num_eval,state_dis,regret_col,num_curve,outcome) #plots segments
    ...
    #plot segment_lengths_vs_steps
    def monot(self,ds_col,steps_col,num_eval,state_dis,regret_col,num_curve,outcome,show=0):
    ...
    def file_location(self):
        strategy_option = ['greedy','random'] #[1: random | 0: greedy]
        if self.epsilon == 0 or self.epsilon == 1:
            strategy = strategy_option[self.epsilon]

        #common_location
        common_loc = f'ran_grd/stop_{self.num_stop}/step_{self.max_esteps}/{strategy}' #random/greedy
        # common_loc = f'eps_grd/stop_{self.num_stop}/step_{self.max_esteps}/{self.epsilon}' #eps-greedy
        # common_loc = f'eps_dcy/stop_{self.num_stop}/step_{self.max_esteps}/{self.epsilon}' #eps-decay
    ...
    def check_convergence(self,final_dataset):
        ...
        if len(state_sizes_list) > 1:
            ...
            for cnt,pair in enumerate(zip(state_sizes_list,state_sizes_list[1:])):
                ...
                opt_list.append(single_opt)

            if len(set(opt_list)) and list(set(opt_list))[0] == 1: #check_if_all_trajectories_are_optimal
...
if __name__ == '__main__':
    ...
    hardns = otdd(
        ...
        setting=3, # [0:sparse | 1:dense | 2:single | 3:stochastic | 4:sinks | 5: strictly-single] rewards_setting
