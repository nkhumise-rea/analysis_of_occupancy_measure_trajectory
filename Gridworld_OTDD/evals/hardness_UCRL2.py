...
from envs.UCRL2 import gridworld

class otdd():
    def __init__(self,
                 test_freq=50,states_size=[3,3],setting=1,n_eps=1000,delta=0.1,stop_cri=50,max_esteps=60):
    ...
    def file_location(self):
        ...
        if self.delta == 0.1:
            ...
            ## ----- Normal_operation -----
            #common_location
            common_loc = f'ucrl_ucrl/stop_{self.num_stop}/step_{self.max_esteps}' #UCRL
            ...
            common_loc = f'ucrl_multiruns/stop_{self.num_stop}/step_{self.max_esteps}' #ucrl
            ...
            #common_location
            common_loc = f'ucrl_hyprm/stop_{self.num_stop}/step_{self.max_esteps}' #UCRL
            
            #curve_datasets
            data_loc = f"{common_loc}/delta_{self.delta}/dts/{self.setting}/ss_{self.state_label()}" 
            # print('data_loc: \n', data_loc)

            #curve_images
            image_loc = f"{common_loc}/delta_{self.delta}/vld/{self.setting}/ss_{self.state_label()}"                   
            # print('image_loc: \n', image_loc)
