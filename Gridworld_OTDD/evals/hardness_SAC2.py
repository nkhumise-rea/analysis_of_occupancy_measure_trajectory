...
from envs.SAC_discrete import gridworld
...
class otdd():
    ...
    def monot(self,ds_col,steps_col,num_eval,state_dis,regret_col,num_curve,outcome,show=0):
        ...
        plt.vlines(x = num_eval, #convergence_line
                   ...
                   colors='black', 
                   ls=':',)
        plt.xlabel('updates (steps)') #episodes
        ...
        ## regret_plot
        plt.subplot(2,2,2)
        x_values = np.arange(len(regret_col)) + 500 +1 #number_of_initial_exploration: 500
        plt.title('Regret Plot')
        plt.plot(x_values,regret_col,'-',color='green',alpha=0.2)
        plt.plot(x_values,self.running_avg(regret_col,0.9),'-',color='orchid')
        plt.vlines(x = num_eval, #convergence_line
                   ymin=min(regret_col),
                   ymax=max(regret_col), 
                   colors='black', 
    ...
    def file_location(self):
        ...
        ## ----- Normal_operation -----      
        #common_location
        common_loc = f'sac_sac/stop_{self.num_stop}/step_{self.max_esteps}' #SAC
        ...
        common_loc = f'sac_multiruns/stop_{self.num_stop}/step_{self.max_esteps}' #sac

        #curve_datasets
        data_loc = f"{common_loc}/runs_9/dts/{self.setting}/ss_{self.state_label()}" 
        # print('data_loc: \n', data_loc)

        #curve_images
        image_loc = f"{common_loc}/runs_9/vld/{self.setting}/ss_{self.state_label()}"                   
        # print('image_loc: \n', image_loc)
...
if __name__ == '__main__':
    ...
    hardns = otdd(
        ...
        max_esteps=40 #max_steps_per_episode
        ) 
