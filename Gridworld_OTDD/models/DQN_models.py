...
from envs.agent.dqn_nn import DQN
...
class gridworld():
    def __init__(self, 
        ...
        self.n_rollouts = 500 #0 #1 6 10 20 50 100 (500) 
    ...
    def policy_data_generation(self,iteration=0, problem_setting='stc'):
        ...
        file_location = f'../envs/policy_models/DQN/set_{self.problem_setting}/iter_{self.iteration}'
    ...
    def occupancy_generation(self,iteration=0, problem_setting='stc'):
        ...
        # print('self.problem_setting: ', self.problem_setting)
        # print('self.iteration: ', self.iteration)
        file_location = f'policy_data/DQN/set_{self.problem_setting}/iter_{self.iteration}'
        ...
        final_policy_occup,_= self.occupancy_measure_finite_timeless(
                        final_policy_data_num,
                        file_location
                        )
        
        initial_policy_occup,_= self.occupancy_measure_finite_timeless(
        ...
        for update in range(len(os.listdir(file_location))-1):
            ...
            policy_occup_1,_= self.occupancy_measure_finite_timeless(
                            update,
                            file_location
                            )
            
            policy_occup_2,_= self.occupancy_measure_finite_timeless(
                            update+1,
                            file_location
                            )
    ...
    def policy_trajectory_evaluation_stats(self,problem_setting='stc'):
        ...
        for iteration in range(10):
            ...
            file_location = f'policy_traj/DQN/set_{problem_setting}' #file_location        
            file_path = abspath(join(this_dir,file_location,file_name)) #file_path: location + name
            data = np.load(file_path, allow_pickle=True) 
    ...
    def policy_evolution_plot(self,iteration=0,problem_setting='stc'):
        ...
        plt.ylabel('relative dis_to_opt',fontweight='bold') #fontsize=16
    ...
    def occupancy_testing(self,iteration=0, problem_setting='stc'):
        ...
        occupancy_optimal, avg_eps_length_optimal = self.occupancy_measure_finite_timeless(
                                  optimal_policy_data_num,
                                  file_location
                                  )
        ...
        for update in range(len(os.listdir(file_location)) - 1): #executes_optimal_policy_model
            ...
            occupancy, avg_eps_length = self.occupancy_measure_finite_timeless(
                                    update,
                                    file_location
                                    )
        ...
        plt.plot(value_relative_error,'k',alpha=0.1)
        # plt.plot(self.rolling(value_relative_error),'m')
        plt.plot(self.running_avg(value_relative_error),'r')
        # plt.title('Relative Value Error')
        plt.ylabel('Rel. Error %',fontweight='bold',fontsize=16) #fontsize=16
    ...
    def save_policy_traj_data(self,meta_data):
        ...
        file_location = f'policy_traj/DQN/set_{self.problem_setting}' #file_location
    ...
    def save_traj_data(self,meta_data,steps):
        ...
        file_location = f'policy_data/DQN/set_{self.problem_setting}/iter_{self.iteration}' #file_location
        os.makedirs(join(this_dir,file_location), exist_ok=True) #create_file_location_if_nonexistent
    ...
    def state_value_function_finite(self,steps,file_location): #using_finite_episodes
        ...
        tx1s = np.asarray(data[4]) #len(states)_of_runs
        ty1s = np.asarray(data[5]) #len(actions)_of_runs

        # num_episodes/rollouts
        # tx1 = tx1s[:self.n_rollouts]
        # ty1 = ty1s[:self.n_rollouts]
        tx1 = tx1s #[:self.n_rollouts]
        ty1 = ty1s #[:self.n_rollouts]
...
if __name__ == '__main__':
    ...
    for i in [2,4,5,6,7,8,9]: #range(10):
    ...
    agent.occupancy_testing(iteration=1, #
                            problem_setting='stc' #['stc','dns','sps']
                                )
    # """
    ...
    agent.policy_evolution_plot(iteration=3,
                                problem_setting='stc' #['stc','dns','sps']
                                )
    # """
    ...
    agent.policy_trajectory_evaluation(iteration=1,
                                problem_setting='stc' #['stc','dns','sps']
                                )
    # """