...
class gridworld():
    ...
    def policy_data_generation(self,iteration=0, problem_setting='stc'):
        ...
        file_location = f'../envs/policy_models/UCRL/set_{self.problem_setting}/iter_{self.iteration}'
        ...
        for step in range(len(os.listdir(file_location))):
            ...
            self.pi_k = self.retrieve_policy_model(step,file_location)
    ...
    def occupancy_generation(self,iteration=0, problem_setting='stc'):
        ...
        # print('self.problem_setting: ', self.problem_setting)
        # print('self.iteration: ', self.iteration)
        file_location = f'policy_data/UCRL/set_{self.problem_setting}/iter_{self.iteration}'
    ...
    def policy_trajectory_evaluation_stats(self,problem_setting='stc'):
        ...
        for iteration in range(10):
            ...
            file_location = f'policy_traj/UCRL/set_{problem_setting}' #file_location        
            file_path = abspath(join(this_dir,file_location,file_name)) #file_path: location + name
            data = np.load(file_path, allow_pickle=True) 

            updates_file_location = f'policy_data/UCRL/set_{problem_setting}/iter_{iteration}'
            ...
            if iteration == 0 or iteration == 2 or iteration == 4 or iteration == 7 :
    ...
    def occupancy_testing(self,iteration=0, problem_setting='stc'):
        ...
        plt.plot(value_relative_error,'k')
        # plt.plot(self.rolling(value_relative_error),'m')
        # plt.title('Relative Value Error')
        plt.ylabel('Rel. Error %',fontweight='bold') #fontsize=16
        plt.xlabel('#updates',fontweight='bold')
        plt.show()
    ...
    def save_policy_traj_data(self,meta_data):
        ...
        file_location = f'policy_traj/UCRL/set_{self.problem_setting}' #file_location
    ...
    def save_traj_data(self,meta_data,steps):
        ...
        file_location = f'policy_data/UCRL/set_{self.problem_setting}/iter_{self.iteration}' #file_location
        os.makedirs(join(this_dir,file_location), exist_ok=True) #create_file_location_if_nonexistent
    ...
    def retrieve_policy_model(self,steps,file_location):
        
        file_name = f'model_{steps}.npy'     
        file_path = abspath(join(this_dir,file_location,file_name)) #file_path: location + name

        #policy_model
        pi_k = np.load(file_path, allow_pickle=True)  
        return pi_k
