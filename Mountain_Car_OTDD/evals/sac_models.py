...
from envs.agents.sac.actor import Actor_Eval
...
class setting():
    ...
    def policy_data_generation(self,iteration=0):
        ...
        #file_location
        # file_location = f'sac_data/models/iter_{self.iteration}'
        file_location = f'/mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/models/iter_{self.iteration}'
        ...
        # for step in [1,10300,21343]: #range(23620,len(os.listdir(file_location))-1):
            # print('step: ', step)
    ...
    def occupancy_generation(self,iteration=0):
        ...
        # """
        # execution        
        file_location = f'/mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/samples/iter_{self.iteration}'
        ...
        """        
        #testing
        file_location = f'sac_data/samples/iter_{self.iteration}'
        final_policy_data_num = 21343 #optimal_policy
        initial_policy_data_num = 1 #initial_policy
    ...
    def policy_trajectory_evaluation(self,iteration=0):
        ...
        max_iterations = [21500,26000,25500,19000,21500]
        num_updates = max_iterations[self.iteration]
        file_name = f'traj_till{num_updates}.npy'
        file_location = f'sac_data/trajectory/iter_{self.iteration}' #file_location        
        ...
        plt.plot(ups,omr,'m')
        plt.ylabel('OMR',fontweight='bold')
        plt.xlabel('updates',fontweight='bold')
        plt.title(f'OMR-Behaviour [{self.algo}-iter{self.iteration}]',fontweight='bold')
        plt.ylim(0.4,0.6)
        plt.show()    
        return
    ...
    def save_images_for_paper(self,iteration=0):
        ...
        # print('max(radii): ', max(radii))
        # print('max(chords): ',max(chords))
        # xxxx
    ...
    def get_action_stochastically(self, state):
        with torch.no_grad():
            action,_ = self.pi_net(state) #incorrect_policy_model
    ...
    def save_policy_trajectories_per_update(self,meta_data,update):
        ...
        # file_location = f'sac_data/paths/iter_{self.iteration}' #file_location

        file_location = f'/mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/cums_reduced/iter_{self.iteration}' #file_location
        # /mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/cums/iter_
    ...
    def save_policy_trajectories(self,meta_data):
        ...
        file_location = f'/mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/paths/iter_{self.iteration}' #file_location
        # /mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/paths/iter_
    ...
    def save_state_action_samples(self,meta_data,steps):
        ...
        # file_location = f'sac_data/samples/iter_{self.iteration}' #file_location

        file_location = f'/mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/samples/iter_{self.iteration}' #file_location
        # /mnt/parscratch/users/acp21rmn/Mountain_Car_OTDD/algo_data/sac/samples/iter_
...
if __name__ == '__main__':
    ...
    agent.performance_test(iteration=0)
    ...
    agent.policy_trajectory_evaluation(iteration=4)
    # """

    """
    # Visualizing Policy_trajectory
    agent.policy_evolution_plot(iteration=4)
    # """

    # """
    # Save Policy_trajectory file (pdf)
    agent.save_images_for_paper(iteration=4) #paper iteration = 4
    # """